...
namespace
{
		...
		struct RegexSearchItem
		{
				RegexSearchItem()
				{}

				RegexSearchItem(const wstring& searchTerm, const Style& style) 
		...
		typedef vector<RegexSearchItem> RegexSearchItems;
		static RegexSearchItems searchItems;

		Style ToStyle(PyObject* pyOb)
		{
				return Style(L"dim", L"default", L"default"); // cjy	    
		}
		...
		static void ParseDictionary(PyObject* dictionary)
		{
				...
				for(int i = 0; i < PyList_Size(keys); ++i)
				{
						...
						for(int j = 0; j < PyList_Size(subKeys); ++j)
						{
								...
								searchItems.push_back(RegexSearchItem(itemName, wsSearchTerm, searchTermValue));
						}	    
				}
		}
}
...
{    
		...
		if(PyList_Check(messageData))
		{
				...
				for(RegexSearchItems::const_iterator it = searchItems.begin();
								it != searchItems.end();
								++it)
				{    
